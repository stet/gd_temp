...
int opts_init(opts_p opts)
{
	...
	opts->input = NULL;
	opts->input_count = 0;
	opts->threads = 1;  // Default to 1 thread for vanity address generation
	opts->case_insensitive = 0;  // Default to case-sensitive for vanity address generation
...
int opts_get(opts_p opts, int argc, char *argv[])
{
	...
	else if (strcmp(opts->command, "vanity") == 0)
	{
		// Add vanity-specific options
		opts_add((struct opt_info){"case-insensitive", "i"}, no_argument);
		opts_add((struct opt_info){"threads", "t"}, required_argument);
	}
...
int opts_process_long(opts_p opts, const char *optname, char *optarg)
{
	...
	else if (strcmp(optname, "case-insensitive") == 0)
	{
		opts->case_insensitive = 1;
	}
	else if (strcmp(optname, "threads") == 0)
	{
		opts->threads = atoi(optarg);
		if (opts->threads < 1)
		{
			error_log("Thread count must be greater than 0");
			return -1;
		}
	}
	else
	{
		error_log("Unknown option: %s", optname);
		return -1;
	}
